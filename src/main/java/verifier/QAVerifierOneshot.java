...
public class QAVerifierOneshot extends AbstractLogVerifier {

	public QAVerifierOneshot(String logfd) {
	  super(logfd);
  }
  ...
  // 0: concurrent
	...
	protected List<long[]> MakeConstraintColumns(Set<Constraint>  constraintColumnsBefore, PrecedenceGraph g) {
        //重複なしのColumnsの作り方
        List<long[]> constraintColumnsAfter = new ArrayList<long[]>();
        for (Constraint constraint : constraintColumnsBefore) {
		long[][] edgesetBefore = constraint.getTxidOfEdges(g);
            if (constraintColumnsAfter.size() == 0) {
                constraintColumnsAfter.add(edgesetBefore[0]);
                constraintColumnsAfter.add(edgesetBefore[1]);
            } else {
                boolean containEdge = false;
                for (long[] edgeAfter : constraintColumnsAfter) {
                    if (edgeAfter[0] == edgesetBefore[0][0] && edgeAfter[1] == edgesetBefore[0][1]) {
                        containEdge = true;
                        break;
                    }
                }
                if (containEdge == false) {
                    constraintColumnsAfter.add(edgesetBefore[0]);
                }

                containEdge = false;
                for (long[] edgeAfter : constraintColumnsAfter) {
                    if (edgeAfter[0] == edgesetBefore[1][0] && edgeAfter[1] == edgesetBefore[1][1]) {
                        containEdge = true;
                        break;
                    }
                }
        ...
        return constraintColumnsAfter;
    }

    private long[][] ChangeListToArray(List<long[]> constraintColumnsAfter){
        // 配列の作成とListからの変換
        long[][] constraintColumnsArray = new long[constraintColumnsAfter.size()][2];
        constraintColumnsAfter.toArray(constraintColumnsArray);


        return constraintColumnsArray;
    }


    	private int[][] MakeConstraintTable(long[][] constraintColumnsArray, Set<Constraint>  constraintColumnsBefore, PrecedenceGraph g){
        //以下からTableを作成する．
        int[][] constraintTable = new int[constraintColumnsBefore.size()][constraintColumnsArray.length];
        int constraintCount = 0;
        for(Constraint constraintBefore : constraintColumnsBefore){
		long[][] constraint = constraintBefore.getTxidOfEdges(g);
            for(int i=0; i<constraintColumnsArray.length; i++){
                if(constraint[0][0]==constraintColumnsArray[i][0] && constraint[0][1]==constraintColumnsArray[i][1]
                        || constraint[1][0]==constraintColumnsArray[i][0] && constraint[1][1]==constraintColumnsArray[i][1]){
                    constraintTable[constraintCount][i] = 1;
                }else{
                    constraintTable[constraintCount][i] = 0;
                }
            }
            constraintCount += 1;
        }
        ...
        for(int i=0; i<constraintTable.length; i++){
            sbTable.append("cons "+ i + ": ");
            for(int j=0; j<constraintTable[i].length; j++){
                sbTable.append(constraintTable[i][j] + " ");
            }
            sbTable.append("\n");
        }
        System.out.println("constraint Table");
	...
	private boolean EncodeAndSolve() {
		...
		// added part 
		List<long[]> constraintColumnsAfter = MakeConstraintColumns(cons, m_g);	
        	long[][] constraintColumnsArray = ChangeListToArray(constraintColumnsAfter);
		int[][] constraintTable = MakeConstraintTable(constraintColumnsArray, cons, m_g);
		convertJSON(constraintColumnsArray, constraintTable);
